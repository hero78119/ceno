...
use multilinear_extensions::virtual_poly::{
    build_eq_x_r_vec, build_eq_x_r_vec_sequential, par_eq_naive, seq_eq_naive,
};
...
fn build_eq_fn(c: &mut Criterion) {
    for nv in NV {
        ...
        group.bench_function(
            BenchmarkId::new("build_eq", format!("par_naive_nv_{}", nv)),
            |b| {
                b.iter_custom(|iters| {
                    let mut time = Duration::new(0, 0);
                    for _ in 0..iters {
                        let instant = std::time::Instant::now();
                        let _ = par_eq_naive(&r, GoldilocksExt2::ONE);
                        let elapsed = instant.elapsed();
                        time += elapsed;
                    }
                    time
        ...
        group.bench_function(
            BenchmarkId::new("build_eq", format!("seq_naive_nv_{}", nv)),
            |b| {
                b.iter_custom(|iters| {
                    let mut time = Duration::new(0, 0);
                    for _ in 0..iters {
                        let instant = std::time::Instant::now();
                        let _ = seq_eq_naive(&r, GoldilocksExt2::ONE);
                        let elapsed = instant.elapsed();
                        time += elapsed;
                    }
                    time
